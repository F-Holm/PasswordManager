...
DataBlock::DataBlock(const size_t largo, char *str)
    : largo(largo), str(new char[largo]) {
  std::memcpy(this->str, str, largo);
}

DataBlock::DataBlock(const std::string &str)
    : largo(str.size()), str(new char[largo]) {
  std::memcpy(this->str, (&str[0]), str.size());
}

DataBlock::DataBlock(const DataBlock &other)
    : largo(other.largo), str(new char[largo]) {
