...
bool DB<T>::UpdateNext() {
    if (IsEOF() || !file.write(reinterpret_cast<const char*>(&element), sizeof(element))) {
        return false;
    }
    file.flush();
    return true;
...
void DB<T>::UpdateCount() {
    ...
    count = total_bytes <= 0 ? 0 : static_cast<std::size_t>(total_bytes / sizeof(element));
}

template <typename T>
bool DB<T>::IsEOF() {
    std::size_t current_element = static_cast<std::size_t>(file.tellg()) / sizeof(element);
